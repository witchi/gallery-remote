...
class StreamDemultiplexor implements GlobalConstants {
	...
	static {
		 ...
		 * StreamDemultiplexor class be gc'd. This will not usually happen, unless
		 * the stuff is being run in an Applet or similar environment where
		 * multiple classloaders are used to load the same class multiple times.
		 * However, even in those environments it's not clear that this here will
		 * do us any good, because classes aren't usually gc'd unless their
		 * classloader is, but the timer thread keeps a reference to the
		 * classloader, and hence ought to prevent the classloader from being
		 * gc'd.
		 */
		cleanup = new Object() {
	 ...
	 * 
	 * @param protocol
	 *           the protocol used on this stream.
	 * @param sock
	 *           the socket which we're to demux.
	 * @param connection
	 *           the http-connection this socket belongs to.
	 */
	 ...
	 * 
	 * @param resp
	 *           the response structure requesting the stream
	...
	/**
	 * Restarts the timer thread that will close an unused socket after 60
	 * seconds.
	 */
	void restartTimer() {
		if (Timer != null)
	...
	int read(byte[] b, int off, int len, ResponseHandler resph, int timeout) throws IOException {
		...
		synchronized (this) {
			...
			if (resph.resp.cd_type != CD_HDRS)
				Log.write(Log.DEMUX, "Demux: Reading for stream " + resph.stream.hashCode());
			...
			try {
				...
				if (timeout != cur_timeout) {
					Log.write(Log.DEMUX, "Demux: Setting timeout to " + timeout + " ms");
	...
	/**
	 * skips a number of bytes in the master stream. This is done via a dummy
	 * read, as the socket input stream doesn't like skip()'s.
	 */
	...
	/**
	 * Determines the number of available bytes. If <var>resph</var> is null,
	 * return available bytes on the socket stream itself (used by
	 * HTTPConnection).
	 */
	 ...
	 * 
	 * @param exception
	 *           the IOException to be sent to the streams.
	 * @param was_reset
	 *           if true then the exception is due to a connection reset;
	...
	synchronized void close(IOException exception, boolean was_reset) {
		...
		Log.write(Log.DEMUX, "Demux: Closing all streams and socket (" + this.hashCode() + ")");
	 ...
	 * 
	 * @param exception
	 *           the exception that led to this call.
	 * @param was_reset
	 *           this flag is passed to the RetryException and is used by the
	 *           RetryModule to distinguish abnormal closes from expected closes.
	 */
	private void retry_requests(IOException exception, boolean was_reset) {
		...
		while (resph != null) {
			/*
			 * if the application is already reading the data then the response has
			 * already been handled. In this case we must throw the real exception.
			 */
	...
	/**
	 * Closes the associated stream. If this one has been markedForClose then the
	 * socket is closed; else closeSocketIfAllStreamsClosed is invoked.
	 */
	private void close(ResponseHandler resph) {
		synchronized (RespHandlerList) {
			...
			Log.write(Log.DEMUX, "Demux: Closing stream " + resph.stream.hashCode());
	 ...
	 * We only want to close the socket if we've been told to do so.
	 * 
	 * <P>
	 * Also note that there might still be responses in the list after the
	 * markedForClose one. These are due to us having pipelined more requests to
	 * the server than it's willing to serve on a single connection. These
	 * requests will be retried if possible.
	 */
	 ...
	 * 
	 * @param response
	 *           the Response after which the connection should be closed.
	 */
		...
		synchronized (RespHandlerList) {
			...
			for (resph = (ResponseHandler) RespHandlerList.enumerate(); resph != null; resph = (ResponseHandler) RespHandlerList.next()) {
				if (resph.resp == resp) // new resp precedes any others
				{
					...
					Log.write(Log.DEMUX, "Demux: stream " + resp.inp_stream.hashCode() + " marked for close");
			...
			Log.write(Log.DEMUX, "Demux: stream " + lasth.stream.hashCode() + " marked for close");
		}
	}
	...
	void abort() {
		...
		synchronized (RespHandlerList) {
			for (ResponseHandler resph = (ResponseHandler) RespHandlerList.enumerate(); resph != null; resph = (ResponseHandler) RespHandlerList
					.next()) {
			...
			/*
			 * Close the socket. Note: this duplicates most of close(IOException,
			 * boolean). We do *not* call close() because that is synchronized, but
			 * we want abort() to be asynch.
			 */
			if (Sock != null) {
				try {
					try {
...
/**
 * This thread is used to reap idle connections. It is NOT used to timeout reads
 * or writes on a socket. It keeps a list of timer entries and expires them
 * after a given time.
 */
class SocketTimeout extends Thread {
	...
	class TimeoutEntry {
		...
		TimeoutEntry next = null, prev = null;
			...
			synchronized (time_list) {
				...
				for (TimeoutEntry entry = time_list[current].next; entry != time_list[current]; entry = entry.next) {
					if (entry.alive && !entry.hyber) {
						...
						entry.kill();
						/*
						 * put on death row. Note: we must not invoke markForClose()
						 * here because it is synch'd and can therefore lead to a
						 * deadlock if that thread is trying to do a reset() or kill()
						 */
						entry.next = marked;
						marked = entry;
						entry = prev;
