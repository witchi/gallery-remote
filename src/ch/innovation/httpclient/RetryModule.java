			...
			try {
				synchronized (re.first) {
					...
					for (RetryException e = re.first; e != null; e = e.next) {
						...
						/*
						 * Don't retry if either the sequence is not idempotent (Sec
						 * 8.1.4 and 9.1.2), or we've already retried enough times, or
						 * the headers have been read and parsed already
						 */
						if (!seq.isIdempotent(req) || (con.ServProtVersKnown && con.ServerProtocolVersion >= HTTP_1_1 && req.num_retries > 0)
						...
						/*
						 * If we have an entity then setup either the entity-delay or
						 * the Expect header
						 */
						...
						/*
						 * If the next request in line has an entity and we're talking
						 * to an HTTP/1.0 server then close the socket after this
						 * request. This is so that the available() call (to watch for
						 * an error response from the server) will work correctly.
						 */
						 ...
						 * problem is that if the server for some reason decides not
						 * to use persistent connections and it does not do a correct
						 * shutdown of the connection, then the response will be
						 * ReSeT. If we did pipeline then we would keep falling into
						 * this trap indefinitely.
						 * 
						 * Note that for HTTP/1.0 servers, if they don't support
						 * keep-alives then the normal code will already handle this
						 * accordingly and won't pipe over the same connection.
						 */
						...
						Log.write(Log.MODS, "RtryM: Retrying request '" + req.getMethod() + " " + req.getRequestURI() + "'");
