 ...
 */
public class GRAppletMini extends GRApplet implements GalleryRemoteCore,
	...
	public void initUI() {
		...
		if (appletFontSize != 0) {
			...
			Log.log(Log.LEVEL_TRACE, MODULE, "Overriding font size to "
					+ appletFontSize);
			Log.log(Log.LEVEL_TRACE, MODULE, "Default font size "
					+ UIManager.getFont("TitledBorder.font").getSize());
			Font f = UIManager.getFont("TitledBorder.font").deriveFont(
					(float) appletFontSize);
			Log.log(Log.LEVEL_TRACE, MODULE,
					"Checking font size " + f.getSize());
		...
		if (!inProgress && hasHadPictures) {
			...
			try {
				...
				if (!(gallery.getComm(null) instanceof GalleryComm2_5)
						&& gallery.getType() != Gallery.TYPE_APPLET) {
					getAppletContext().showDocument(
							new URL(getCodeBase().toString()
									+ "add_photos_refresh.php"), "hack");
				}
	...
	protected void jbInit() {
		...
		jScrollPane.setBorder(new TitledBorder(BorderFactory
		...
		jResize.setToolTipText(GRI18n.getString(MODULE,
				"ResizeBeforeUploadHelp"));
		...
		jInspector.add(captionLabel, new GridBagConstraints(0, 0, 1, 1, 1.0,
		...
		try {
			...
			call = jsObject.getMethod("call", new Class[] { String.class,
					Object[].class });
	...
	public void jbUpdate() {
		...
		if (extrafields != null) {
			...
			while (i.hasNext()) {
				...
				jExtrafield.setBackground(UIManager
						.getColor("TextField.inactiveBackground"));

				jInspector.add(new JLabel(name + ":"), new GridBagConstraints(
						0, j++, 1, 1, 1.0, 0.0, GridBagConstraints.WEST,
						GridBagConstraints.HORIZONTAL, new Insets(0, 5, 0, 0),
						0, 0));
				jInspector.add(jExtrafield, new GridBagConstraints(0, j++, 1,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == jAdd) {
			...
			gallery.doUploadFiles(new UploadProgress(DialogUtil
					.findParentWindow(this)) {
				@Override
				public void doneUploading(String newItemName, Picture picture) {
					g2Feedback("uploadedOne", new Object[] { newItemName,
							picture.toString() });
				}
			});
			...
			if (e.getDocument() == jCaption.getDocument()) {
				...
				while (i.hasNext()) {
					...
					if (e.getDocument() == jExtrafield.getDocument()) {
						p.setExtraField(jExtrafield.getName(),
								jExtrafield.getText());
	...
	public void g2Feedback(String method, Object[] params) {
		if (gallery.getGalleryVersion() == 2) {
			try {
				Log.log(Log.LEVEL_TRACE, MODULE, "Invoking Javascript method '"
						+ method + "' with " + Arrays.asList(params) + " on "
						+ window);
