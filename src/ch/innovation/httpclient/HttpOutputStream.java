 ...
 * request will be sent with the corresponding Content-length header and
 * anything written to the stream will be written on the socket immediately.
 * This is the preferred way. If the stream is created without a length then one
 * of two things will happen: if, at the time of the request, the server is
 * known to understand HTTP/1.1 then each write() will send the data immediately
 * using the chunked encoding. If, however, either the server version is unknown
 * (because this is first request to that server) or the server only understands
 ...
 * cause the HttpOutputStream to buffer all the data instead, sending it only
...
public class HttpOutputStream extends OutputStream {
	 ...
	 * 
	 * @param length
	 *           the number of bytes which will be sent over this stream
	 */
	 ...
	 * 
	 * @param req
	 *           the request this stream is to be associated with
	 * @param os
	 *           the underlying output stream to write our data to, or null if we
	 *           should write to a ByteArrayOutputStream instead.
	 * @param con_to
	 *           connection timeout to use in sendRequest()
	 */
	...
	/**
	 * Reset this output stream, so it may be reused in a retried request. This
	 * method may only be invoked by modules, and <strong>must</strong> never be
	 ...
	 * 
	 * @param buf
	 *           an array containing the data to write
	 * @param off
	 *           the offset of the data whithin the buffer
	 * @param len
	 *           the number bytes (starting at <var>off</var>) to write
	 * @exception IOException
	 *               if any exception is thrown by the socket, or if writing
	 *               <var>len</var> bytes would cause more bytes to be written
	 *               than this stream is willing to accept.
	 ...
	 * 
	 * @exception IOException
	 *               if any exception is thrown by the underlying socket, or if
				...
				if (length == -1) {
					if (Log.isEnabled(Log.CONN) && trailers.length > 0) {
						...
						for (int idx = 0; idx < trailers.length; idx++)
							Log.write(Log.CONN, "       " + trailers[idx].getName() + ": " + trailers[idx].getValue());
					}
