...
public class SlideshowFrame extends PreviewFrame implements Runnable, PreferenceNames, CancellableTransferListener, MouseMotionListener {
	...
	public void showSlideshow() {
		try {
			if (GalleryRemote.IS_MAC_OS_X) {
				// on the Mac, using a maximized window doesn't take care of the
				// menu bar
	...
	public void run() {
		...
		while (sleepTime > 0 && running && !shutdown) {
			...
			try {
				...
				// Log.log(Log.LEVEL_TRACE, MODULE, "sleepTime: " + sleepTime +
				// " - " + (System.currentTimeMillis() - pictureShownTime));
				// Thread.sleep(1000);
	...
	class SlideshowPane extends JPanel implements ActionListener {
		...
		private static final long serialVersionUID = 2411062309343175891L;
