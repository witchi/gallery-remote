...
/**
 * A simple enumerator which delegates everything to the real enumerator. If a
 * CIString element is returned, then the string it represents is returned
 * instead.
 */
final class CIHashtableEnumeration implements Enumeration<Object> {
	Enumeration<Object> HTEnum;

	public CIHashtableEnumeration(Enumeration<Object> enum1) {
		HTEnum = enum1;
	}
...
/**
 * This class' raison d'etre is that I want to use a Hashtable using Strings as
 * keys and I want the lookup be case insensitive, but I also want to be able
 * retrieve the keys with original case (otherwise I could just use
 * toLowerCase() in the get() and put()). Since the class String is final we
 * create a new class that holds the string and overrides the methods hashCode()
 * and equals().
 */
