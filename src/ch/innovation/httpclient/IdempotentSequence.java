...
/**
 * <P>
 * This class checks whether a sequence of requests is idempotent. This is used
 * to determine which requests may be automatically retried. This class also
 * serves as a central place to record which methods have side effects and which
...
class IdempotentSequence {
	...
	private Hashtable<String, Object> threads = new Hashtable<String, Object>();
	 ...
	 * two threads having the same request-URI. Each thread is then marked as
	 * idempotent or not according to the following rules:
	 * 
	 * <OL>
	 ...
	 * <LI>else, if the first method has side effects and is complete then the
	 * thread is idempotent;
	 * <LI>else, if the first method has side effects, is not complete, and no
	 * other method has side effects then the thread is idempotent;
