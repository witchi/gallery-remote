...
public class AboutPanel extends JComponent implements AnimationModel {
	...
	private ArrayList<String> tokenizeText() {
		...
		StringTokenizer st = new StringTokenizer(GalleryRemote.instance().properties.getProperty("aboutText"), "\n");
	...
	public void paintComponent(Graphics g) {
		...
		String version = GalleryRemote.instance().properties.getProperty("version");
		g.drawString(version, (getWidth() - fm.stringWidth(version)) / 2, getHeight() - 5);

		g = g.create((getWidth() - maxWidth) / 2, this.top, maxWidth, getHeight() - this.top - this.bottom);
	...
	public void paintAnimation(int scrollPosition) {
		...
		repaint(getWidth() / 2 - maxWidth, this.top, maxWidth * 2, getHeight() - this.top - this.bottom);
	}
