...
class SocksClient {
	 ...
	 * 
	 * @param host
	 *           the host the SOCKS server is sitting on.
	 * @param port
	 *           the port the SOCKS server is listening on.
	 */
	 ...
	 *           the port the SOCKS server is listening on.
	 * @param version
	 *           the version the SOCKS server is using.
	 * @exception SocksException
	 *               if the version is invalid (Currently allowed are: 4 and 5).
	 */
	...
	Socket getSocket(String host, int port, InetAddress localAddr, int localPort) throws IOException {
		...
		try {
			Log.write(Log.SOCKS, "Socks: contacting server on " + socks_host + ":" + socks_port);
			...
			case -1:
				...
				try {
					...
					Log.write(Log.SOCKS, "Socks: V4 request failed: " + se.getMessage());
	...
	private void v4ProtExchg(InputStream inp, OutputStream out, String host, int port) throws SocksException, IOException {
		...
		Log.write(Log.SOCKS, "Socks: Beginning V4 Protocol Exchange for host " + host + ":" + port);
		...
		Log.write(Log.SOCKS, "Socks: Sending connect request for user " + new String(user, 0, user.length - 1));
		...
		if (version == -1)
			...
			Log.write(Log.SOCKS, "Socks: Warning: received version 4 " + "instead of 0");
		else if (version != 0)
			throw new SocksException("Received invalid version: " + version + "; expected: 0");

		int sts = inp.read();

		Log.write(Log.SOCKS, "Socks: Received response; version: " + version + "; status: " + sts);
	...
	private void v5ProtExchg(InputStream inp, OutputStream out, String host, int port) throws SocksException, IOException {
		...
		Log.write(Log.SOCKS, "Socks: Beginning V5 Protocol Exchange for host " + host + ":" + port);

		// send version 5 verification methods

		Log.write(Log.SOCKS, "Socks: Sending authentication request; methods" + " No-Authentication, Username/Password");
		...
		Log.write(Log.SOCKS, "Socks: Received response; version: " + version + "; method: " + method);
	 ...
	 * 
	 * NOTE: this is not implemented currently. Will have to wait till Java
	 * provides the necessary access to the system routines.
	 */
	...
	private void negotiate_userpwd(InputStream inp, OutputStream out) throws SocksException, IOException {
		...
		Log.write(Log.SOCKS, "Socks: Entering authorization subnegotiation" + "; method: Username/Password");

		// get username/password

		AuthorizationInfo auth_info;
		try {
			auth_info = AuthorizationInfo.getAuthorization(socks_host, socks_port, "SOCKS5", "USER/PASS", null, null, true);
		...
		Log.write(Log.SOCKS, "Socks: Sending authorization request for user " + user_str);
