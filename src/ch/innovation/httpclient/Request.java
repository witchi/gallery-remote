...
public final class Request implements RoRequest, Cloneable {
	...
	public Request(HTTPConnection con, String method, String req_uri, NVPair[] headers, byte[] data, HttpOutputStream stream,
			boolean allow_ui) {
	 ...
	 *           the request headers
	 * @param data
	 *           the entity as a byte[]
	 * @param stream
	 *           the entity as a stream
	 * @param allow_ui
	...
	public void setRequestURI(String req_uri) {
		if (req_uri != null && req_uri.trim().length() > 0) {
			...
			if (req_uri.charAt(0) != '/' && !req_uri.equals("*") && !method.equals("CONNECT") && !isAbsolute(req_uri))
				req_uri = "/" + req_uri;
	...
	private static final boolean isAbsolute(String uri) {
		...
		while (pos < len && (ch = uri.charAt(pos)) != ':' && ch != '/' && ch != '?' && ch != '#')
			pos++;
	...
	/**
	 * @param stream
	 *           an output stream on which the entity is written
	 */
	...
	/**
	 * @param allow_ui
	 *           are modules and handlers allowed to popup windows or otherwise
