 ...
 * applet to easily run as an application. The only thing it can't do is access
 * URL's. Applet parameters are entered on the command line with name as one
 * word and value as the next.
 * 
 * @version 1.1
...
public class DummyAppletContext extends Frame implements AppletStub,
	 ...
	 * 
	 * @param args the command line arguments. Contains possibly height and
	 * width, and any applet parameters
	 * 
	...
	/**
	 * Parse the command line arguments. Get the initial width and height of the
	 * window if specified (-width [value] -height [value]), and the applet
