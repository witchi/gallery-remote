...
public class URI {
	...
	protected static final Hashtable<String, Integer> defaultPorts = new Hashtable<String, Integer>();
	protected static final Hashtable<String, Boolean> usesGenericSyntax = new Hashtable<String, Boolean>();
	protected static final Hashtable<String, Boolean> usesSemiGenericSyntax = new Hashtable<String, Boolean>();
	...
	static {
		...
		/*
		 * Note: schemes which definitely don't use the generic-URI syntax and
		 * must therefore never appear in the above list: "urn", "mailto", "sdp",
	 ...
	 *               if no scheme can be found or a specified port cannot be
	 *               parsed as a number
	 */
	 ...
	 * @exception ParseException
	 *               if <var>base</var> is null and <var>rel_uri</var> is not an
	 *               absolute URI, or if <var>base</var> is not null and the
	 *               scheme is not known to use the generic syntax, or if a given
	 ...
	 *           the URL
	 * @exception ParseException
	 *               if <code>url.toExternalForm()</code> generates an invalid
	 ...
	 * 
	 * @see <A
	 *      HREF="http://www.ics.uci.edu/pub/ietf/uri/rfc2396.txt">rfc-2396</A>
	 */
	...
	public boolean equals(Object other) {
		if (other instanceof URI) {
			...
			type == SEMI_GENERIC && areEqual(userinfo, o.userinfo) && areEqualIC(host, o.host) && port == o.port && areEqual(path, o.path) ||

			type == GENERIC && areEqual(userinfo, o.userinfo) && areEqualIC(host, o.host) && port == o.port && pathsEqual(path, o.path)
					&& areEqual(query, o.query) && areEqual(fragment, o.fragment)));
		}
		...
		if (other instanceof URL) {
			...
			type == SEMI_GENERIC && areEqualIC(h, o.getHost()) && (port == o.getPort() || o.getPort() == defaultPort(scheme))
					&& areEqual(f, o.getFile()) ||

			type == GENERIC && areEqualIC(h, o.getHost()) && (port == o.getPort() || o.getPort() == defaultPort(scheme))
	...
	public int hashCode() {
		if (hashCode == -1)
			hashCode = (scheme != null ? unescapeNoPE(scheme, null).hashCode() : 0)
					+ (type == OPAQUE ? (opaque != null ? unescapeNoPE(opaque, null).hashCode() : 0) * 7 : (host != null ? unescapeNoPE(host,
							null).toLowerCase().hashCode() : 0)
							* 7
							+ (path != null ? unescapeNoPE(path, null).hashCode() : 0)
							* 13
	...
	public static char[] escape(char[] elem, BitSet allowed_char, boolean utf8) {
		...
		for (int idx = 0, pos = 0; idx < elem.length; idx++) {
			...
			if (allowed_char.get(c))
				...
				/*
				 * We're UTF-8 encoding the chars first, as recommended in the HTML
				 * 4.0 specification:
				 * http://www.w3.org/TR/REC-html40/appendix/notes.html#h-B.2.1 Note
				 * that this doesn't change things for ASCII chars
				 */
				if (c <= 0x007F) {
					pos = enc(tmp, pos, c);
				} else if (c <= 0x07FF) {
	 ...
	 * @exception ParseException
	 *               if the two digits following a `%' are not a valid hex number
	 */
		...
		for (int sidx = 0; sidx < buf.length; sidx++) {
			if (buf[sidx] == '%') {
				...
				try {
					...
					/*
					 * Hmm, people not reading specs again, so we just ignore it...
	...
	public static void main(String args[]) throws Exception {
		...
		/*
		 * Relative URI test set, taken from Section C of rfc-2396 and Roy's
		 * test1. All Roy's URI parser tests can be found at
		 * http://www.ics.uci.edu/~fielding/url/ The tests have been augmented by
		 * a few for the IPv6 syntax
		 */
		...
		testParser(base, "g:h", "g:h");
		/*
		 * we have to skip these, as usesGeneraicSyntax("fred") returns false and
		 * we therefore don't parse relative URI's here. But test5 is the same
		 * except that the http scheme is used. testParser(base, "g",
		 * "fred:///s//a/b/g"); testParser(base, "./g", "fred:///s//a/b/g");
		 * testParser(base, "g/", "fred:///s//a/b/g/"); testParser(base, "/g",
		 * "fred:///g"); testParser(base, "//g", "fred://g"); testParser(base,
		 * "//g/x", "fred://g/x"); testParser(base, "///g", "fred:///g");
		 * testParser(base, "./", "fred:///s//a/b/"); testParser(base, "../",
		 * "fred:///s//a/"); testParser(base, "../g", "fred:///s//a/g");
		 * testParser(base, "../../", "fred:///s//"); testParser(base, "../../g",
		 * "fred:///s//g"); testParser(base, "../../../g", "fred:///s/g");
	...
	private static void testParser(URI base, String relURI, String result) throws Exception {
		if (!(new URI(base, relURI).toExternalForm().equals(result))) {
			throw new Exception("Test failed: " + nl + "  base-URI = <" + base + ">" + nl + "  rel-URI  = <" + relURI + ">" + nl
					+ "  expected   <" + result + ">" + nl + "  but got    <" + new URI(base, relURI) + ">");
		}
	}
	...
	private static void testUnescape(String escaped, String raw) throws Exception {
		if (!unescape(escaped, null).equals(raw))
			throw new Exception("Test failed: " + nl + "  escaped-string: " + escaped + nl + "  unescaped:      " + unescape(escaped, null)
					+ nl + "  expected:       " + raw);
	}
}
